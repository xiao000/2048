...
//得到每一个grid和cell的宽高，并设置它的位置与宽高
function setWidthHeight(){
	var width = window.innerWidth;
	var height = window.innerHeight-$('#scoreId').height();

	// 5*base_marginLeft+4*cell_width = width
	// base_marginLeft = cell_width/4;

	 base_marginLeft = (1/21)*width;
	 base_marginTop = (1/21)*height;

	 cell_width = 4*base_marginLeft;
	 cell_height = 4*base_marginTop;
	console.log('cell_width is '+cell_width
	+" cell_height is "+cell_height);
	...
	for(var r=0;r<4;r++)
	{
		...
		{
			//console.log($('[id $= "'+r+c+'"]'));
			//设置id尾部为01,02..,31,32..的元素 设置top和left
			$('[id $= "'+r+c+'"]').css({
...
var game={
	...
	/*	funGet("gridPanel").style.width=this.CN*116+16+"px";
		//设置id为gridPanel的高为: RN*116+16+"px";
		funGet("gridPanel").style.height=this.RN*116+16+"px";*/
		var grids=[];
		var cells=[];
		//r从0开始,到<RN结束，每次++
		...
		funGet("gridPanel").innerHTML=grids.join("")
			                    +cells.join("");
		setWidthHeight();
	},
	start:function(){//开始游戏方法
		...
		funGet("top").innerHTML=this.getTop();
		this.state=this.RUNNING;//初始化游戏状态为运行
		funGet("gameOver").style.display="none";
		...
		$('#gridPanel').on('swipeLeft', function () {
			console.log('swipe left');
			this.moveLeft();
		}.bind(this))

		$('#gridPanel').on('swipeRight', function () {
			console.log('swipe right');
			this.moveRight();
		}.bind(this))

		$('#gridPanel').on('swipeUp', function () {
			console.log('swipe up');
			this.moveUp();
		}.bind(this))

		$('#gridPanel').on('swipeDown', function () {
	...
	updateView:function(){
		...
		funGet("score").innerHTML=this.score;
		//span对象
		...
		//	找到id为finalScore的span，设置内容为游戏的分数
			funGet("finalScore").innerHTML=this.score;
		//	找到id为gameOver的div，显示出来
			funGet("gameOver").style.display="block";
